...
object ActorGroupConfig {
  ...
  private val kamonConfig = defaultConfig.getConfig("kamon")
  ...
  private val groupFilters = {
    if(kamonConfig.hasPath(ActorGroup)) {
      val cfg = kamonConfig.getConfig(ActorGroup)
